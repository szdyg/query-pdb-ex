...
std::map<std::string, int64_t> pdb_parser::get_symbols(const std::set<std::string> &names) const { return call_with_pdb_stream(get_symbols_impl, names); }

std::map<std::string, std::map<std::string, field_info>> pdb_parser::get_struct(const std::map<std::string, std::set<std::string>> &names) const {
  return call_with_pdb_stream(get_struct_impl, names);
}

std::map<std::string, std::map<std::string, int64_t>> pdb_parser::get_enum(const std::map<std::string, std::set<std::string>> &names) const {
  return call_with_pdb_stream(get_enum_impl, names);
}

std::map<std::string, int64_t> pdb_parser::get_symbols_impl(const PDB::RawFile &raw_file, const PDB::DBIStream &dbi_stream, const PDB::TPIStream &tpi_stream,
...
std::map<std::string, std::map<std::string, field_info>> pdb_parser::get_struct_impl(const PDB::RawFile &raw_file, const PDB::DBIStream &dbi_stream,
                                                                                     const PDB::TPIStream &tpi_stream, const std::map<std::string, std::set<std::string>> &names) {
  ...
  for (const auto &record : tpi_stream.GetTypeRecords()) {
    if (record->header.kind == PDB::CodeView::TPI::TypeRecordKind::LF_STRUCTURE) {
      ...
      if (auto it = names.find(leaf_name); it != names.end()) {
        std::map<std::string, field_info> fields = get_struct_single(tpi_stream, type_record, it->second);
...
std::map<std::string, field_info> pdb_parser::get_struct_single(const PDB::TPIStream &tpi_stream, const PDB::CodeView::TPI::Record *record, const std::set<std::string> &names) {
  ...
  for (size_t i = 0; i < maximum_size;) {
    ...
    if (field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_BCLASS && field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_VFUNCTAB &&
        field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_NESTTYPE && field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_ENUM &&
        field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_MEMBER && field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_STMEMBER &&
        field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_METHOD && field_record->kind != PDB::CodeView::TPI::TypeRecordKind::LF_ONEMETHOD) {
    ...
    if (field_record->kind == PDB::CodeView::TPI::TypeRecordKind::LF_MEMBER) {
      ...
      if (names.find(leaf_name) != names.end()) {
        field_info info{};
        info.offset = offset;
        if (referenced_type && referenced_type->header.kind == PDB::CodeView::TPI::TypeRecordKind::LF_BITFIELD) {
...
std::map<std::string, std::map<std::string, int64_t>> pdb_parser::get_enum_impl(const PDB::RawFile &raw_file, const PDB::DBIStream &dbi_stream, const PDB::TPIStream &tpi_stream,
  ...
  for (const auto &record : tpi_stream.GetTypeRecords()) {
    if (record->header.kind == PDB::CodeView::TPI::TypeRecordKind::LF_ENUM) {
      ...
      if (auto it = names.find(leaf_name); it != names.end()) {
        std::map<std::string, int64_t> fields = get_enum_single(type_record, GetLeafSize(static_cast<PDB::CodeView::TPI::TypeRecordKind>(record->data.LF_ENUM.utype)), it->second);
        result.insert({it->first, fields});
      }
    }
  }
...
std::map<std::string, int64_t> pdb_parser::get_enum_single(const PDB::CodeView::TPI::Record *record, uint8_t underlying_type_size, const std::set<std::string> &names) {
  std::map<std::string, int64_t> result;
